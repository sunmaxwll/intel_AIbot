...
echo Tiny Yolo Object detection Mode|rosrun sound_play say.py

gnome-terminal -x bash -c "roslaunch realsense_camera r200_nodelet_rgbd.launch"

sleep 1

gnome-terminal -x bash -c "roslaunch movidius_ncs_launch ncs_camera.launch cnn_type:=tinyyolo_v1 camera:=others input_topic:=/camera/rgb/image_raw "
sleep 1
